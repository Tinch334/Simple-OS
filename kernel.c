...
void kernel_entry(void) {
    __asm__ __volatile__(
        "csrw sscratch, sp\n" //Store stack pointer in sscratch, the supervisor scratch register.
        "addi sp, sp, -4 * 31\n" //Allocate space in stack and store all registers.
        "sw ra,  4 * 0(sp)\n"
        "sw gp,  4 * 1(sp)\n"
        "sw tp,  4 * 2(sp)\n"
        "sw t0,  4 * 3(sp)\n"
        ...
        "mv a0, sp\n"
        "call handle_trap\n" //Store SP in a0 and call trap handler.
...
struct process *create_process(uint32_t pc) {
    ...
    // Stack all calee-saved registers. This are the values that will be restored in the first context switch.
